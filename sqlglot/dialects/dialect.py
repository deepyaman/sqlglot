...
from sqlglot.time import TIMEZONES, apply_time_role, format_time, subsecond_precision
...
def _with_time_roles(inverse_mapping: dict[str, str]) -> dict[str, str]:
    # Derive the context-free inverses of the role-explicit month/day parse tokens from the
    # dialect's padded inverse (its %m), so the write side never has to know the expression's
    # role. Lenient parse defaults to padded too, but a dialect whose lenient token differs
    # (e.g. Spark parses with the non-padded M) overrides %mparse.
    for field in ("m", "d"):
        padded = inverse_mapping.get(f"%{field}") or f"%{field}"

        inverse_mapping.setdefault(f"%{field}parsestrict", padded)
        inverse_mapping.setdefault(f"%{field}parse", padded)
...
# Their format gets the lenient/strict parse roles; everything else gets the format roles.
...
def build_formatted_time(
    ...
    def _builder(args: BuilderArgs, dialect: Dialect) -> E:
        ...
        format_expr = target_dialect.format_time(fmt)
        if isinstance(format_expr, exp.Literal) and format_expr.is_string:
            parse = exp_class in PARSE_TIME_EXPRESSIONS
            format_expr = exp.Literal.string(apply_time_role(format_expr.this, parse))

        return exp_class(this=seq_get(args, 0), format=format_expr)
